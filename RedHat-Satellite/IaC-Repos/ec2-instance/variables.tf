...
variable "os_type" {
  description = "Operating system type (Amazon Linux, Ubuntu, Windows)"
  type        = string
  validation {
    condition     = contains(["Amazon Linux", "Ubuntu", "Windows"], var.os_type)
    error_message = "OS type must be one of Amazon Linux, Ubuntu, or Windows"
  }
}

variable "default_ami" {
  description = "Default AMI to use if OS type is not in the map"
  type        = string
}
...
variable "custom_user_data" {
  description = "Custom user data script for initialization"
  type        = string
  default     = ""
}

variable "additional_ebs_volumes" {
  description = "List of additional EBS volumes with encryption"
  type = list(object({
    device_name = string
    volume_type = string
    volume_size = number
